...
# all = si un des bloc de la l'iterable est false alors false. Il faut quils soit tous vrais pour true.
# any = Si un bloc est true dedans, alors true.

def give_bmi(height: list[int | float], weight: list[int | float]) -> list[int | float]:
	try:
		if not isinstance(height, list) or not all(isinstance(i, (int, float)) for i in height):
			raise AssertionError("height should take only integer or float.")
		if not isinstance(weight, list) or not all(isinstance(i, (int, float)) for i in weight):
